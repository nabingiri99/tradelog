...
export default function TradeLog() {
  ...
  function handlePrintReport() {
    ...
    const breaks = closed.filter((t) => t.result === "BreakEven").length;
    const winRate = closed.length > 0 ? (wins / closed.length) * 100 : 0;
    const totalR = closed.reduce((s, t) => {
      if (t.result === "Win") return s + t.rr;
      if (t.result === "Loss") return s - 1;
      return s;
    }, 0);
    const netPnl = filtered.reduce((s, t) => s + (t.pnlAmount ?? 0), 0);

    const rangeDesc = [fromDate && `from ${fromDate}`, toDate && `to ${toDate}`]
      .filter(Boolean)
      .join(" ");

    const rows = filtered
      .map((t) => {
        const dirColor = t.direction === "Buy" ? "#047857" : "#be123c";
        const resColor =
          t.result === "Win"
            ? "#047857"
            : t.result === "Loss"
        ...
        return `<tr>
          ...
          <td>${t.entryTime ? escapeHtml(t.entryTime) : "—"}</td>
          <td>${escapeHtml(t.pair)}</td>
          <td style="color:${dirColor};font-weight:600">${t.direction}</td>
          <td style="text-align:right">${t.entry.toFixed(t.entry >= 100 ? 1 : 4)}</td>
          <td style="text-align:right">${t.stopLoss.toFixed(t.stopLoss >= 100 ? 1 : 4)}</td>
          <td style="text-align:right">${t.target.toFixed(t.target >= 100 ? 1 : 4)}</td>
          <td style="color:${resColor};font-weight:600">${t.result}</td>
          <td style="text-align:right">${(t.rr ?? 0).toFixed(2)}R</td>
        </tr>`;
      })
      .join("");
...
<head>
<meta charset="utf-8" />
<title>TradeLog Report — ${new Date().toISOString().slice(0, 10)}</title>
<style>
...
<body>
  <h1>TradeLog Journal Report</h1>
  <p class="sub">Generated ${new Date().toLocaleString()} · ${filtered.length} trade${filtered.length === 1 ? "" : "s"} shown${rangeDesc ? ` · ${rangeDesc}` : ""}</p>
  <div class="stats">
    <div class="stat"><div class="l">Trades</div><div class="v">${filtered.length}</div></div>
    <div class="stat"><div class="l">Win Rate</div><div class="v">${closed.length > 0 ? winRate.toFixed(1) + "%" : "—"}</div></div>
    <div class="stat"><div class="l">W / L / BE</div><div class="v">${wins} / ${losses} / ${breaks}</div></div>
    <div class="stat"><div class="l">Net R</div><div class="v">${totalR >= 0 ? "+" : ""}${totalR.toFixed(2)}</div></div>
    <div class="stat"><div class="l">Net P&L ($)</div><div class="v">${netPnl > 0 ? "+" : ""}${netPnl.toLocaleString(undefined, { maximumFractionDigits: 2 })}</div></div>
  </div>
  <table>
    <thead>
      <tr>
        <th>Date</th><th>Time</th><th>Pair</th><th>Dir</th>
        <th style="text-align:right">Entry</th><th style="text-align:right">SL</th><th style="text-align:right">TP</th>
        <th>Result</th><th style="text-align:right">R:R</th>
      </tr>
    </thead>
    <tbody>${rows}</tbody>
  </table>
  <p class="foot">Generated by TradeLog — export to PDF via the browser print dialog.</p>
</body>
</html>`);
    win.document.close();
    win.focus();
